...
export function App() {
  ...
  async function handleReloadAfterConflict() {
    ...
    try {
      ...
      setError(null) // clears a stale failed-reload message from an earlier failed attempt
    } catch {
  ...
  // While the conflict modal is open, the rest of the editor is marked `inert` (mouse/keyboard
  // unreachable, hidden from assistive tech) so SaveStatus's own Tab-trap inside the dialog is
  // backed by a real modality barrier, not just a visual one.
  const backgroundInert = saveState === 'conflict'
  ...
  return (
    <div className="editor-shell">
      ...
      <div className="editor-content" inert={backgroundInert}>
        ...
        {error && <p role="alert">{error}</p>}
        {/* General store error (EditorCanvas geometry / PrintGroupPanel failures), independent of
            SaveStatus's save-specific message; suppressed while saveState is 'invalid' so the same
            text is never rendered by both this banner and SaveStatus at once. */}
        {saveState !== 'invalid' && saveError && <p role="alert">{saveError}</p>}
      </div>
    </div>
  )
}
